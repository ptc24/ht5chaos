...
function readform() {
	...
	julia = document.getElementById("julia").checked ? 1 : 0;
	jx = parseFloat(document.getElementById("jx").value);
	jy = parseFloat(document.getElementById("jy").value);
	readuncontroversial();
}
...
function render(withprev) {
	...
	for(var i=0;i<canvas.width;i++) {
		for(var j=0;j<canvas.height;j++) {
			...
			var mag = Math.sqrt(((xx-ccx)*(xx-ccx))+((yy-ccy)*(yy-ccy)))*2;
